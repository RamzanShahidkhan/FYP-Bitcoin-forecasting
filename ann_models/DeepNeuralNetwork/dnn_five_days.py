...
from ann_models.feedForwardNetwork.pre_process import create_train_test
...
def prepare_model(minute,closep1,openp,lowp, data, timestamp):
    ...
    try:
        ...
        l = Dense(11, activation='linear', kernel_initializer='normal', kernel_constraint=maxnorm(3))(main_input)
   ...
   # write_predicted_data(ts_date, actual1, pred1)
    ...
    plt.grid(True)
    plt.title("Bitcoin forecast for " +str(minute) +" minutes (DNN)", fontweight='bold')
    plt.legend(loc=2)
    plt.xlabel("Time")
...
def write_predicted_data(timestamp ,actual,predicted):
    ...
    file = open('predicted_data_days.csv', 'w')
...
def write_errors_in_file( rmse_value,mae_value,mape_value):
    # open a file for writing.
    file = open('ffnn_days_errors_values.csv', 'w')
    # create the csv writer object.
    mywriter = csv.writer(file)
...
def data_load_fun():
    ...
    t,p, a = prepare_model(minute,closep1,openp,lowp, data, timestamp)
    ## print(" ended!!!! ")
    return t, p, a
