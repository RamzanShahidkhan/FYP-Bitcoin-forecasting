...
#from sklearn.metrics import mean_squared_error, classification_report # additional
...
from ann_models.DeepNeuralNetwork.utils import create_Xt_Yt
...
STEP = 1
FORECAST = 10
data_original = pd.read_csv('btceUSD_1-min_data_2012-01-01_to_2017-05-31.csv')
...
def experiment(params):
    ...
    plt.plot(preData, label="predictive data")
    plt.plot(orgData , label =" original data")
    plt.legend()
    plt.show()
    ...
    if np.isnan(mse):
        print ('NaN happened')
        print ('-' * 10)
        return {'lossM': 999999, 'status': STATUS_OK}
