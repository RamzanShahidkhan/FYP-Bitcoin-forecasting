...
def data_sets(labels, closep1, p_sets, y_i, timestamp):
    for i in range(1, len(p_sets), 1):
        ...
        prepare_model(i, label,data_length, closep1, tr_date, ts_date, X_train, X_test, Y_train, Y_test)
        # plt.pause(0.5)
        # time.sleep(0.5)
        # plt.close()


def prepare_data(closep1, data_original, timestamp):
    ...
    data_sets(p_sets_labels, closep1, x_i, y_i, timestamp)
    print("prepare-data ended !!")


def prepare_model(i, label,d_length, closep1, tr_date, ts_date, x_train, x_test, y_train, y_test):
    ...
    model.add(LSTM(11, input_shape=(1,d_length), activation='linear'))
    # model.add(LSTM(4))
    ...
    plt.grid(True)
    plt.title("RNN set " + str(i) + "" + str(label), fontweight='bold')
    plt.legend(loc=2)
    plt.xlabel("Time")
...
def data_load_fun():
    ...
    prepare_data(closep1, data, timestamp)
    ## print(" ended!!!! ")
    # return t, p, a
