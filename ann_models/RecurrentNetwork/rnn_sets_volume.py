...
def data_sets(labels, volumep1, p_sets, y_i, timestamp):
    for i in range(1, len(p_sets), 1):
        ...
        prepare_model(i, label,data_length, volumep1, tr_date, ts_date, X_train, X_test, Y_train, Y_test)
        # plt.pause(0.5)
        # time.sleep(0.5)
        # plt.close()
...
def prepare_model(i, label,d_length, volumep1, tr_date, ts_date, x_train, x_test, y_train, y_test):
